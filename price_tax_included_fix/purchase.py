...
#    Copyright (C) 2004-2010 Oliver Yuan (<http://www.openstone.cn>).
#
...
class purchase_order(osv.osv):
    ...
    def _prepare_order_line_move(self, cr, uid, order, order_line, picking_id, group_id, context=None):
        ...
        print price_unit_exclude_tax

        if order_line.product_uom.id != order_line.product_id.uom_id.id:
            price_unit_exclude_tax *= order_line.product_uom.factor / order_line.product_id.uom_id.factor
        if order.currency_id.id != order.company_id.currency_id.id:
            #we don't round the price_unit, as we may want to store the standard price with more digits than allowed by the currency
            price_unit_exclude_tax = self.pool.get('res.currency').compute(cr, uid, order.currency_id.id, order.company_id.currency_id.id, price_unit_exclude_tax, round=False, context=context)
        res = []
